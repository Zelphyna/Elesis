# Design

Confirmed design direction:

- Elesis should read as her base Elsword version, not a Grand Chase variant.
- Avoid advanced Elsword class-path details such as Grand Master, Blazing Heart, Bloody Queen, Empire Sword, or Flame Lord.
- Visual style should be simplified and adapted to Slay the Spire 2: dark fantasy, painterly, readable silhouettes, rough brush texture, and controlled crimson accents.
- Gameplay identity should start from sword-focused offense with restrained fire motifs, not from full evolution-class spectacle.
- The first implemented card pool uses prototype-safe card effects grouped by Vitality, Destruction, Flame, and Parry. Chivalry and Flame are implemented as counter powers; Parry is still a design label for efficient defensive cards.
- Card portraits have a first functional art pass. The 40 implemented cards each have runtime and big portrait PNGs named after their card IDs, with source art grouped by current mechanics and rarity beats.
- Character selection uses a separated background and Elesis character layer. The current background avoids the Belder/Velder fortress backdrop and instead uses a mostly unified crimson/dark painterly field with abstract sword shapes so Elesis can be positioned independently from the description text. The active character-select layer uses the cleaned pose 1 from `Elesis/images/versions/character-select/separated-v3/poses/`; earlier chroma-key and checkerboard attempts were rejected because transparent character assets must not keep visible colored fringes.
- The starter relic visual is the Belder Knight Emblem: a red-and-gold knight crest with a claymore motif on a transparent background, matching her red knight identity.
- Energy UI uses a red crystal-and-gold knight orb. Chivalry uses a red-gold sword crest power icon, and Flame uses a sharper red-orange sword-flame crest icon.
- Map marker art in solo and multiplayer reuses Elesis' compendium icon so her map presence matches the character icon.
- Combat, shop, and rest-site character art use transparent sprite assets with bottom-aligned scene placement. Combat art leans more chibi/readable than the larger menu illustrations so her feet align cleanly with other player visuals; the shop scene reuses the combat-scale pose and follows the current specialization.
- Rest-site art for every Elesis form should show her seated as if on the existing campfire log, without drawing the log into the asset. Her sword should be straight and vertical beside her, with the blade tip down and the grip/pommel up.
- Specialization and base-form art lives under `Elesis/images/specializations/` as transparent sprites. Base Elesis uses `base/`; specialization art is grouped by path and numbered step, such as `saber_knight_path/1_saber_knight/`, `saber_knight_path/2_grand_master/`, and `saber_knight_path/3_empire_sword/`. Scene contexts that vary by form use the same path/step folder structure, for example `Elesis/scenes/rest_site/specializations/saber_knight_path/1_saber_knight/`.
- Specialization and evolution events use dedicated transparent portrait canvases under `Elesis/images/events/` so Elesis sits left of the option list instead of covering it.
- Card portraits use the pre-v0.4.16 first functional art pass rather than the chibi redraw. Keep future card portrait experiments separate until explicitly selected.
- Compendium and run/top-panel icon art uses the restored pre-v0.4.17 Elesis icon.

Do not treat generated art attempts as final until one is explicitly selected as the project asset.
