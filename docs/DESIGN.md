...
- Combat, shop, and rest-site character art use official Elesis portraits when a matching official image exists, with transparent sprite assets and bottom-aligned scene placement. Each scene context owns its scale/offset tuning so the selected form stays close to the size of other STS2 character visuals; combat and shop use the current specialization portrait.
