...
- Character selection uses a separated background and Elesis character layer. The current background avoids the Belder/Velder fortress backdrop and instead uses a mostly unified crimson/dark field with abstract sword shapes so the background and Elesis can be positioned independently from the description text. The active character-select layer uses the cleaned pose 1 from `Elesis/images/versions/character-select/separated-v3/poses/`; earlier chroma-key and checkerboard attempts were rejected because transparent character assets must not keep visible colored fringes.
