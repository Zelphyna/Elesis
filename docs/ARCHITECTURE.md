...
- `ElesisCode/Specializations/`: run-local specialization flow for Elesis. Belder Knight Emblem uses STS2 model hooks to check pending progression whenever Elesis arrives on the map and after the custom XP reward is taken; `ElesisSpecializationController` also registers a lightweight `RunManager.RoomEntered` fallback because map-entry model hooks can be inconsistent across STS2 builds. The controller opens the specialization event at 15 XP and evolution events at 35 and 55 XP. Threshold checks are idempotent: if an event did not actually complete, the next map arrival can retry it. The custom XP reward can open progression immediately after selection because STS2 may go from reward screen directly to the next map vote without entering a `MapRoom`. Runtime character IDs can include BaseLib prefixes such as `ELESIS-ELESIS`, so progression identifies Elesis runs by the owned Belder Knight Emblem instead of comparing character ID strings. Map entry also has a one-time combat XP fallback for cases where the custom reward was not selected or not injected.
