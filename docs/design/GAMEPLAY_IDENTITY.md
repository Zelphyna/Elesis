...
- Belder Knight Emblem tracks run-local specialization experience. It starts at 0 each run. Combat XP is offered as a selectable post-combat reward: normal combats grant 2 XP, elites grant 3 XP, and bosses grant 5 XP. Non-combat nodes grant 1 XP when the player leaves the node and returns to the map. At 15 XP, once the map is reopened, Elesis chooses Saber Knight, Pyro Knight, Dark Knight, or Soar Knight for the rest of the run.
