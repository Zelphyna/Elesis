...
- Belder Knight Emblem tracks run-local specialization experience. It starts at 0 each run, gains 2 XP after normal combat nodes, 3 XP after elite nodes, and 1 XP after other completed map nodes. At 15 XP, once the map is reopened, Elesis chooses Saber Knight, Pyro Knight, Dark Knight, or Soar Knight for the rest of the run.
- Current specialization effects are intentionally modest first-pass bonuses: Saber Knight adds +1 to Chivalry gains, Pyro Knight adds +1 to Flame gains, Dark Knight adds +1 attack damage, and Soar Knight adds +1 Block gained.
